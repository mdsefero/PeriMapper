...
![PeriMapper dashboard](docs/dashboard.png)

## Features

- Interactive Houston-area ZIP-level choropleth driven by a perinatal pickle.
- Binary variables shown as prevalence percentages; continuous variables shown as ZIP-level means.
- Configurable thresholds for minimum effective count and minimum records per ZIP.
- Downloadable summary table alongside the map.
...
- `perimapper_core.py`: Shared analytical helpers used by the dashboard.
...
- A ZIP lookup CSV at `data/zip_lookup.csv` by default, overridable via `ZIP_LOOKUP_PATH`.
...
Build the ZIP lookup if `data/zip_lookup.csv` doesn't already exist:
